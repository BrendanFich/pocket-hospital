(function () {
    ...
    $('.next-btn').click(function () {
        ...
        if (btnValue === '下一步，填写手机号') {
            ...
            var phone = $('.phone').val();
            var ssmCode = $('.validnumber').val();

            if (!checkIdCard(idCard)) return;

            if (name && idCard && nation && address && phone && ssmCode) {
...
function checkInput(fa) {
    ...
    console.log('fa', name, idCard, nation, address, phone, ssmCode);
    if (name && idCard && nation && address) {
        if (btnValue === '下一步，填写手机号' || (phone && ssmCode)) {
